...
void auth_akarin_spec_a_init_data_size(obfs *self, server_info_t *server) {
...
void auth_akarin_rand_set_server_info(obfs *self, server_info_t *server) {
...
void auth_akarin_spec_a_set_server_info(obfs *self, server_info_t *server) {
    memmove(&self->server, server, sizeof(server_info_t));
...
unsigned int auth_akarin_get_client_rand_len(auth_akarin_local_data *local, server_info_t *server, int datalength) {
    return local->get_tcp_send_rand_len(local, server, datalength, &local->random_client, local->last_client_hash);
}

unsigned int auth_akarin_get_server_rand_len(auth_akarin_local_data *local, server_info_t *server, int datalength) {
...
int auth_akarin_rand_pack_auth_data(auth_akarin_global_data *global, server_info_t *server, auth_akarin_local_data *local,
