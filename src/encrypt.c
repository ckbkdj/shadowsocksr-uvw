...
static const char* supported_ciphers[CIPHER_NUM] = {
    "none", "table", "rc4", "rc4-md5-6", "rc4-md5", "aes-128-cfb", "aes-192-cfb", "aes-256-cfb",
    "aes-128-ctr", "aes-192-ctr", "aes-256-ctr", "bf-cfb", "camellia-128-cfb", "camellia-192-cfb", "camellia-256-cfb", "cast5-cfb",
    "des-cfb", "idea-cfb", "rc2-cfb", "seed-cfb", "salsa20", "chacha20", "chacha20-ietf"
};

#ifdef USE_CRYPTO_POLARSSL
static const char* supported_ciphers_polarssl[CIPHER_NUM] = { "none",
    "table",
    "ARC4-128",
    "ARC4-128",
    "ARC4-128",
...
static const char* supported_ciphers_mbedtls[CIPHER_NUM] = { "none",
    "table",
    "ARC4-128",
    "ARC4-128",
    "ARC4-128",
...
static const CCAlgorithm supported_ciphers_applecc[CIPHER_NUM] = {
    kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmRC4, kCCAlgorithmRC4, kCCAlgorithmRC4, kCCAlgorithmAES,
    kCCAlgorithmAES, kCCAlgorithmAES, kCCAlgorithmAES, kCCAlgorithmAES, kCCAlgorithmAES, kCCAlgorithmBlowfish,
    kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmCAST, kCCAlgorithmDES, kCCAlgorithmInvalid,
    kCCAlgorithmRC2, kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid
};

static const CCMode supported_modes_applecc[CIPHER_NUM] = { kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCModeRC4,
    kCCModeRC4, kCCModeCFB, kCCModeCFB, kCCModeCFB,
    kCCModeCTR, kCCModeCTR, kCCModeCTR, kCCModeCFB,
    kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCModeCFB,
    kCCModeCFB, kCCModeCFB, kCCModeCFB, kCCAlgorithmInvalid,
    kCCAlgorithmInvalid, kCCAlgorithmInvalid, kCCAlgorithmInvalid };
#endif

static const int supported_ciphers_iv_size[CIPHER_NUM] = { 0, 0, 0, 6, 16, 16, 16, 16, 16, 16, 16, 8, 16, 16, 16, 8, 8, 8, 8, 16, 8, 8, 12 };

static const int supported_ciphers_key_size[CIPHER_NUM] = { 16, 16, 16, 16, 16, 16, 24, 32, 16, 24, 32, 16,
    16, 24, 32, 16, 8, 16, 16, 16, 32, 32, 32 };
...
int brealloc(buffer_t* ptr, size_t len, size_t capacity)
{
    if (ptr == NULL)
        return -1;
    size_t real_capacity = len > capacity ? len : capacity;
...
static int crypto_stream_xor_ic(uint8_t* c, const uint8_t* m, uint64_t mlen, const uint8_t* n, uint64_t ic, const uint8_t* k, int method)
{
    switch (method) {
    case SALSA20:
...
static void merge(uint8_t* left, int llength, uint8_t* right, int rlength, uint32_t salt, uint64_t key)
{
...
int cipher_key_size(const cipher_t* cipher)
{
    ...
    if (cipher->info->base != NULL && POLARSSL_CIPHER_ID_BLOWFISH == cipher->info->base->cipher) {
        return 128 / 8;
    }
...
int bytes_to_key(const cipher_t* cipher, const digest_type_t* md, const uint8_t* pass, uint8_t* key)
{
    size_t datal;
...
void cipher_context_init(cipher_env_t* env, cipher_ctx_t* ctx, int enc)
{
...
void cipher_context_set_iv(cipher_env_t* env, cipher_ctx_t* ctx, uint8_t* iv, size_t iv_len, int enc)
{
    ...
    if (cc->valid == kCCContextValid) {
        ...
        CCCryptorStatus ret;
        ret = CCCryptorCreateWithMode(cc->encrypt, cc->mode, cc->cipher, cc->padding, cc->iv, cc->key, cc->key_len, NULL, 0, 0,
            kCCModeOptionCTR_BE, &cc->cryptor);
        if (ret != kCCSuccess) {
            if (cc->cryptor != NULL) {
                CCCryptorRelease(cc->cryptor);
                cc->cryptor = NULL;
...
#elif defined(USE_CRYPTO_POLARSSL)
    // XXX: PolarSSL 1.3.11: cipher_free_ctx deprecated, Use cipher_free()
    // instead.
...
static int cipher_context_update(cipher_ctx_t* ctx, uint8_t* output, size_t* olen, const uint8_t* input, size_t ilen)
{
#ifdef USE_CRYPTO_APPLECC
    cipher_cc_t* cc = &ctx->cc;
    if (cc->valid == kCCContextValid) {
        CCCryptorStatus ret;
        ret = CCCryptorUpdate(cc->cryptor, input, ilen, output, ilen, olen);
...
#if defined(USE_CRYPTO_OPENSSL)
    int err = 0, tlen = *olen;
    err = EVP_CipherUpdate(evp, (uint8_t*)output, &tlen, (const uint8_t*)input, ilen);
    *olen = tlen;
    return err;
#elif defined(USE_CRYPTO_POLARSSL)
    return !cipher_update(evp, (const uint8_t*)input, ilen, (uint8_t*)output, olen);
#elif defined(USE_CRYPTO_MBEDTLS)
    return !mbedtls_cipher_update(evp, (const uint8_t*)input, ilen, (uint8_t*)output, olen);
#endif
}
...
int ss_encrypt_all(cipher_env_t* env, buffer_t* plain, size_t capacity)
{
    int method = env->enc_method;
    if (method > TABLE) {
        cipher_ctx_t evp;
        ...
        if (method >= SALSA20) {
            crypto_stream_xor_ic((uint8_t*)(cipher->array + iv_len), (const uint8_t*)plain->array, (uint64_t)(plain->len),
                (const uint8_t*)iv, 0, env->enc_key, method);
        } else {
            err = cipher_context_update(&evp, (uint8_t*)(cipher->array + iv_len), &cipher->len, (const uint8_t*)plain->array, plain->len);
        }
...
int ss_encrypt(cipher_env_t* env, buffer_t* plain, enc_ctx_t* ctx, size_t capacity)
{
    if (ctx != NULL) {
        static buffer_t tmp = { 0, 0, 0, NULL };
        ...
        if (env->enc_method >= SALSA20) {
            ...
            crypto_stream_xor_ic((uint8_t*)(cipher->array + iv_len), (const uint8_t*)plain->array, (uint64_t)(plain->len + padding),
                (const uint8_t*)ctx->evp.iv, ctx->counter / SODIUM_BLOCK_SIZE, env->enc_key, env->enc_method);
            ctx->counter += plain->len;
            if (padding) {
                memmove(cipher->array + iv_len, cipher->array + iv_len + padding, cipher->len);
            }
        } else {
            err = cipher_context_update(&ctx->evp, (uint8_t*)(cipher->array + iv_len), &cipher->len, (const uint8_t*)plain->array, plain->len);
            if (!err) {
                return -1;
            }
        }
...
int ss_decrypt_all(cipher_env_t* env, buffer_t* cipher, size_t capacity)
{
    int method = env->enc_method;
    if (method > TABLE) {
        size_t iv_len = env->enc_iv_len;
        ...
        if (method >= SALSA20) {
            crypto_stream_xor_ic((uint8_t*)plain->array, (const uint8_t*)(cipher->array + iv_len), (uint64_t)(cipher->len - iv_len),
                (const uint8_t*)iv, 0, env->enc_key, method);
        } else {
            ret = cipher_context_update(&evp, (uint8_t*)plain->array, &plain->len, (const uint8_t*)(cipher->array + iv_len),
                cipher->len - iv_len);
        }
...
int ss_decrypt(cipher_env_t* env, buffer_t* cipher, enc_ctx_t* ctx, size_t capacity)
{
    if (ctx != NULL) {
        static buffer_t tmp = { 0, 0, 0, NULL };
        ...
        if (env->enc_method >= SALSA20) {
            ...
            if (padding) {
                ...
                memmove(cipher->array + iv_len + padding, cipher->array + iv_len, cipher->len - iv_len);
                sodium_memzero(cipher->array + iv_len, padding);
            }
            crypto_stream_xor_ic((uint8_t*)plain->array, (const uint8_t*)(cipher->array + iv_len),
                (uint64_t)(cipher->len - iv_len + padding), (const uint8_t*)ctx->evp.iv, ctx->counter / SODIUM_BLOCK_SIZE,
                env->enc_key, env->enc_method);
            ctx->counter += cipher->len - iv_len;
            if (padding) {
                memmove(plain->array, plain->array + padding, plain->len);
            }
        } else {
            err = cipher_context_update(&ctx->evp, (uint8_t*)plain->array, &plain->len, (const uint8_t*)(cipher->array + iv_len),
                cipher->len - iv_len);
        }
...
void enc_table_init(cipher_env_t* env, int method, const char* pass)
{
    uint32_t i;
    uint64_t key = 0;
    uint8_t* digest;
...
void enc_key_init(cipher_env_t* env, int method, const char* pass)
{
    if (method < NONE || method >= CIPHER_NUM) {
